  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const message =
        ...
        err?.message ||
        "Impossible de créer le compte. Vérifie les informations fournies.";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto grid max-w-5xl grid-cols-1 gap-10 lg:grid-cols-2">
      <div className="rounded-3xl bg-white p-8 shadow-lg shadow-primary-100/70">
        <div className="mb-6">
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-900">Bienvenue 👋</h1>
          <p className="mt-2 text-slate-600">
            Crée ton compte pour réserver une salle en quelques clics.
          </p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <input
              type="text"
              required
              className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-slate-900 shadow-inner shadow-slate-100 focus:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-100"
          ...
          <button
            type="submit"
            disabled={loading || !!emailError || !!passwordError || !name.trim()}
      ...
      <div className="hidden rounded-3xl bg-gradient-to-br from-primary-600 via-primary-700 to-slate-900 p-8 text-white shadow-xl lg:block">
        ...
        <h2 className="mt-4 text-3xl font-bold">Crée un compte et commence à réserver</h2>
        <p className="mt-3 text-white/80">
          Garde une trace de toutes tes réservations, vérifie les disponibilités en un
          clin d'œil et annule facilement si besoin.
        </p>
        <div className="mt-10 space-y-4">
          <FeatureItem title="Gestion sécurisée (JWT)" />
          <FeatureItem title="Disponibilités mises à jour" />
          <FeatureItem title="Interface moderne" />
        </div>
      </div>
    </div>
  );
