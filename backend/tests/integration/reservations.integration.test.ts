...
describe('Reservations Integration Tests', () => {
  ...
  describe('POST /api/reservations', () => {
    ...
    it('devrait créer une réservation avec des données valides', async () => {
      ...
      expect(response.body).toHaveProperty('message', 'Réservation créée avec succès');
      expect(response.body.reservation).toHaveProperty('id');
      expect(response.body.reservation).toHaveProperty('user_id', userId);
      expect(response.body.reservation).toHaveProperty('room_id', validReservation.roomId);
      expect(response.body.reservation).toHaveProperty('start_time', validReservation.startTime);
      expect(response.body.reservation).toHaveProperty('end_time', validReservation.endTime);
    });
    ...
    it('devrait retourner 400 si roomId manquant', async () => {
      const response = await request(app)
        .post('/api/reservations')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ startTime: validReservation.startTime, endTime: validReservation.endTime });

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('error', 'roomId, startTime et endTime sont requis');
    });
    ...
    it('devrait retourner 400 si startTime manquant', async () => {
      const response = await request(app)
        .post('/api/reservations')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ roomId: validReservation.roomId, endTime: validReservation.endTime });
    ...
    it('devrait retourner 400 si endTime manquant', async () => {
      const response = await request(app)
        .post('/api/reservations')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ roomId: validReservation.roomId, startTime: validReservation.startTime });
    ...
    it('devrait retourner 409 pour un créneau déjà réservé', async () => {
      ...
      expect(response.body).toHaveProperty('error', 'Cette salle est déjà réservée pour ce créneau');
    });
  ...
  describe('DELETE /api/reservations/:id', () => {
    ...
    beforeEach(async () => {
      // Créer une réservation pour les tests de suppression
      ...
      reservationId = createResponse.body.reservation.id;
    });
    ...
    it('devrait supprimer sa propre réservation', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty('message', 'Réservation supprimée avec succès');
    ...
    it('devrait retourner 403 pour supprimer la réservation d\'un autre utilisateur', async () => {
      ...
      expect(response.body).toHaveProperty('error', 'Vous ne pouvez supprimer que vos propres réservations');
    });
  });
  ...
  describe('Workflow complet', () => {
    it('devrait gérer le workflow complet: inscription, login, réservation, consultation, suppression', async () => {
      ...
      const availabilityResponse = await request(app)
        .get('/api/rooms/1/availability?startTime=2026-01-16T09:00:00&endTime=2026-01-16T10:00:00');
      ...
      const availableAgainResponse = await request(app)
        .get('/api/rooms/1/availability?startTime=2026-01-16T09:00:00&endTime=2026-01-16T10:00:00');
