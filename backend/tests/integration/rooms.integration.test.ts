...
describe('Rooms Integration Tests', () => {
  ...
  describe('GET /api/rooms/:id/availability', () => {
    ...
    it('devrait retourner disponible pour une salle sans réservation', async () => {
      const response = await request(app)
        .get(`/api/rooms/1/availability?startTime=${startTime}&endTime=${endTime}`);
    ...
    it('devrait retourner disponible pour un créneau différent', async () => {
      ...
      const response = await request(app)
        .get('/api/rooms/1/availability?startTime=2026-01-15T12:00:00&endTime=2026-01-15T13:00:00');
    ...
    it('devrait retourner indisponible pour un créneau qui chevauche partiellement', async () => {
      ...
      const response = await request(app)
        .get('/api/rooms/1/availability?startTime=2026-01-15T10:30:00&endTime=2026-01-15T11:30:00');
    ...
    it('devrait retourner 400 si startTime manquant', async () => {
      const response = await request(app)
        .get(`/api/rooms/1/availability?endTime=${endTime}`);

      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('error', 'Les paramètres startTime et endTime sont requis');
    });

    it('devrait retourner 400 si endTime manquant', async () => {
      const response = await request(app)
        .get(`/api/rooms/1/availability?startTime=${startTime}`);
    ...
    it('devrait retourner 404 pour une salle inexistante', async () => {
      const response = await request(app)
        .get(`/api/rooms/999/availability?startTime=${startTime}&endTime=${endTime}`);

      expect(response.status).toBe(404);
      expect(response.body).toHaveProperty('error', 'Salle non trouvée');
    });

    it('devrait permettre des réservations consécutives (fin = début)', async () => {
      // Créer une réservation de 10h à 11h
      await request(app)
        .post('/api/reservations')
        .set('Authorization', `Bearer ${authToken}`)
      ...
      // Vérifier la disponibilité de 11h à 12h (juste après)
      const response = await request(app)
        .get('/api/rooms/1/availability?startTime=2026-01-15T11:00:00&endTime=2026-01-15T12:00:00');

      expect(response.status).toBe(200);
      // Le comportement dépend de l'implémentation - la réservation peut être disponible ou non
    });
  });
});
